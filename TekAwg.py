...
def bifloat_to_uint(value, bit_depth):
    ...
    Not a totally straightforward conversion, as the full bit depth of the arb
    is not used in favor of providing meaningful conversion between a float with
    three or four significant figures and a binary value on the AWG.  For
    example, for a 14-bit AWG, the floating-point value 1.0 will convert to the
    highest-used sample code, while 0.998 will convert to the next-highest, etc.
    This implies a loss of about 2.4% of the full-scale AWG resolution in favor of
    making convenient values like 0.5 exactly representable.
